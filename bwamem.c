...
/*
 *	The mem's previous chain filter strategy can't find all bp info, and FMEAS filter strategy  may be better than the previous.
 *	Refined by smith-waterman , those chain can  be extend. So this mem_chain_flt can't modify.
 */
...
/*
 *  In fact , one of the hits is redundant... 
 *                 ****|******************|******
 *                 *******|************************|*****
 */
...
/*
 * 	FMEAS  fliter strategy:
 * 	1. Generate S segment (it is no need to sort).
 * 	2. Caculate FMEAS value.
 * 	3. if FMEAS value > 0.95 , record both part(H/T).
 * 	4. Filtering perfect match ,this function can't applied by samse. The other way ,PE mem_pair process get back this result.
 *
 * 	Implement:
 * 	
...
/*
 *	This previous strategy is unreasonable to find break point.So ignore those code. 
 */
}
